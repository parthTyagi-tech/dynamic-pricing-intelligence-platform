...
export default function CompetitorMatcher() {
  ...
  const handleRunIntelligence = async () => {
    ...
    setLoading(true);
    setPlatforms([]);
    setCrawlStep("Initializing Market Intelligence Engine...");

    try {
      const token = localStorage.getItem("klypup_token");
      const url = `${apiClient.defaults.baseURL}/recommendations/stream-scrape/${selectedProduct.id}`;
      ...
      if (!response.ok) {
        throw new Error(`Failed to initialize streaming: ${response.statusText}`);
      }
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (chunk.status === "started") {
              setCrawlStep(chunk.message);
            } else if (chunk.status === "success" && chunk.data) {
              const platformData = chunk.data;
              setPlatforms(prev => {
                const filtered = prev.filter(p => p.platform_name !== platformData.platform_name);
                return [...filtered, {
                  id: `platform-${filtered.length}`,
                  ...platformData
                }];
              });
              setCrawlStep(`Scraped pricing from ${platformData.platform_name}...`);
            } else if (chunk.status === "error") {
              console.error("Scraper chunk error:", chunk.message);
            }
          } catch (e) {
      ...
      console.error("Error running price intelligence stream:", err);
      alert(err.message || "Failed to scan platforms.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-white/5 border border-white/8 rounded-3xl p-6 relative overflow-hidden"
        ...
        <div className="absolute top-0 right-0 w-32 h-32 opacity-10 blur-3xl bg-gradient-to-br from-[#059669] to-[#047857]" />
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-gradient-to-br from-[#059669]/10 to-[#047857]/10 text-emerald-400 border border-[#059669]/20">
            <Globe size={24} />
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white/5 border border-white/8 rounded-3xl p-6 space-y-5"
            ...
            <div className="relative">
              <button
                ...
                className="w-full flex items-center justify-between bg-white/5 border border-white/10 rounded-xl p-3 text-left cursor-pointer hover:border-[#059669]/40 transition-colors"
              >
                  ...
                  >
                    <div className="p-2 border-b border-white/5">
                      <input
                        ...
                        className="w-full bg-white/5 border border-white/10 text-white text-xs rounded-lg px-3 py-2 outline-none focus:border-[#059669]/50"
                        autoFocus
                      />
                    </div>
                    <div className="max-h-60 overflow-y-auto">
                      ...
                      {filteredProducts.map(p => (
                        <button
                          ...
                          className={`w-full text-left p-3 hover:bg-[#059669]/10 transition-colors border-b border-white/5 last:border-0 cursor-pointer ${
                            selectedProduct?.id === p.id ? "bg-[#059669]/20" : ""
                          }`}
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5">
                                {p.brand && <span className="text-emerald-400">{p.brand}</span>}
                                {p.brand && " · "}
                                {p.category}
                              </p>
                            </div>
                ...
                >
                  <div className="bg-white/[0.03] border border-white/8 rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#059669]/10 text-[#10b981] border border-[#059669]/20 font-mono">
                        {selectedProduct.sku}
                      </span>
                    </div>
                    <h4 className="text-sm font-bold text-white leading-snug">{selectedProduct.name}</h4>
                    {selectedProduct.brand && (
                      <span className="inline-block text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 font-semibold">
                        {selectedProduct.brand}
                      </span>
                    )}
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3.5 bg-[#212121] hover:bg-[#333] text-white text-xs font-bold rounded-xl shadow-lg shadow-black/20 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed select-none transition-all"
            >
              <Zap size={14} />
        ...
        {/* RIGHT — RESULTS */}
        <div className="lg:col-span-2 bg-[#171717]/80 border border-white/5 rounded-[25px] p-6 min-h-[400px] flex items-start justify-center relative overflow-hidden"
             style={{ backdropFilter: "blur(12px)", boxShadow: "0 20px 50px rgba(0,0,0,0.5)" }}>
          ...
          <AnimatePresence mode="wait">
            {!loading && platforms.length === 0 && (
              ...
              >
                ...
                <h4 className="text-sm font-semibold text-slate-400 font-sentient">No price data yet</h4>
                <p className="text-xs text-slate-500 max-w-sm">
                  Select a product from your catalog and run the price intelligence engine to compare prices across Amazon, Flipkart, Walmart, eBay, BestBuy, and Target.
                </p>
              </motion.div>
            )}
            ...
            {(platforms.length > 0 || loading) && (
              ...
              >
                {/* Real-time Loading Banner at the top */}
                {loading && (
                  <div className="flex items-center gap-3 p-4 bg-[#059669]/10 border border-[#059669]/20 rounded-xl">
                    <div className="w-5 h-5 rounded-full border-2 border-[#059669]/20 border-t-[#10b981] animate-spin flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-[#10b981]">Scanning marketplaces in real-time...</p>
                      <p className="text-[10px] text-slate-400 truncate mt-0.5">{crawlStep}</p>
                    </div>
                  </div>
                )}
                {/* Results Header */}
