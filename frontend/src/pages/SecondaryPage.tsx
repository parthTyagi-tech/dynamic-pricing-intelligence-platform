...
export default function SecondaryPage({ kind }: { kind: SecondaryKind }) {
  ...
  const { data } = usePricingData(kind !== "catalog" && kind !== "agents");
  ...
  const onRollback = async (actionId: string) => {
    try {
      await rollbackApproval(actionId);
      push("Price successfully rolled back to pre-approval value.", "success");
      refreshHistory();
    } catch (err: any) {
      push(err.response?.data?.message || "Rollback failed.", "error");
    }
  };
