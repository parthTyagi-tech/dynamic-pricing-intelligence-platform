"""
✅ FIX: Renamed from audit_loging.py -> audit_log.py (typo fixed)
✅ FIX: to_dict() was referencing self.metadata but column is extra_metadata
"""
import uuid
...
class AuditLog(db.Model):
    ...
    # ✅ FIX: Column is extra_metadata, not metadata (metadata is reserved by SQLAlchemy)
    ...
    def to_dict(self):
        return {
            ...
            # ✅ FIX: was self.user.name — guard against None user
            "user_name": self.user.name if self.user else "system",
            "organization_id": self.organization_id,
            "action": self.action,
            "entity_type": self.entity_type,
            "entity_id": self.entity_id,
            # ✅ FIX: was self.metadata (wrong attribute name), now self.extra_metadata
